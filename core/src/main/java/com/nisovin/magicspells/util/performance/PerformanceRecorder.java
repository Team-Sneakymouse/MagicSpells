...
public final class PerformanceRecorder {
    ...
    public enum Counter { INVENTORY_SLOTS, MATCH_CALLS }
    ...
    public Scope enter(String operation, String spell, String detail) {
        ...
        if (stats == null) {
            if (session.stats.size() >= maxKeys) {
                ...
                // Keep context even when the key is dropped: never charge its matches to its parent.
            } else {
                stats = new Stats();
    ...
    public final class Scope implements AutoCloseable {
        ...
        public void add(Counter counter, long amount) {
            if (stats != null && !closed) stats.counters[counter.ordinal()] += amount;
        }
